...
export async function fetchFeaturedPosts() {
  try {
    ...
    const featuredPostsWithImages = featuredPosts.map((post) => {
      ...
      const imagePath = image ? image.id : "1DhVjLMUGn2dDpsG9Ndi2aq9QBlJ9KD0o";
...
export async function fetchPostById(postId) {
  try {
    ...
    const image = post.image
      ? images.find((img) => img.name === post.image)
      : undefined;
    ...
    const imagePath = image?.id
      ...
      : "https://drive.google.com/uc?id=1DhVjLMUGn2dDpsG9Ndi2aq9QBlJ9KD0o";
